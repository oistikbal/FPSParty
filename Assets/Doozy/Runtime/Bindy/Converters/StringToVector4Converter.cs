...
{
    /// <summary>
    /// Converts a string value to a Vector4 value.
    /// The expected string format is "x,y,z,w".
    /// <para/> Note that it throws an ArgumentException if the source or target types are not of the expected type.
    /// </summary>
    public class StringToVector4Converter : IValueConverter
    {
        /// <summary>
        ...
        public bool CanConvert(Type source, Type target) =>
            source == typeof(string) && target == typeof(Vector4);
        ...
        public object Convert(object value, Type target)
        {
            ...
            if (target != typeof(Vector4))
                throw new ArgumentException($"Invalid target type: {target}. Expected: {typeof(Vector4)}.");
